...
dataLoader <- function(league, season = NULL){
  ...
  skaters <-
    ...
    dplyr::mutate(
      `TA_GA` =
        dplyr::case_when(
          giveaways > 0 ~ takeaways/giveaways,
          giveaways == 0 & takeaways > 0 ~ 100,
          TRUE ~ 0
        ),
      posGroup =
        dplyr::case_when(
...
playerCard <- function(chosen, leagueData){
  ...
  statistics <-
    ...
    ggplot2::geom_bar(
      ggplot2::aes(fill = score, group = measure_type),
      stat = "identity",
      position = "dodge"
    ) +
