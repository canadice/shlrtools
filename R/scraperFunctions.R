    ...
    lapply(
      ...
      FUN = function(page){
        paste(baseLink, page, sep = "") %>%
          ...
          rvest::html_attr("href") %>%
          .[
            !stringr::str_detect(., "&action=newpost") &
              !stringr::str_detect(., "&action=lastpost") &
              !stringr::str_detect(., "simulationhockey") &
              !stringr::str_detect(., "&page=2")
          ] %>%
  ...
  function(player){
    ...
    userTags <-
      paste0(
        ...
        paste(teams$team, collapse = "|"),"|","Team [A-z ]+|",
